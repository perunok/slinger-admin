# Slinger Cloud: product vision and original design brief

> **This is the original design brief, kept as background and roadmap.** For what is actually built see the root [`README.md`](../README.md),
> [`server/README.md`](../server/README.md) and [`server/openapi.yaml`](../server/openapi.yaml). Where this document and
> [`api-contract-v2.md`](api-contract-v2.md) differ, the contract wins (auth routes are under `/v1/auth/*`;
> realtime, extension, OIDC and similar items below are roadmap only and are not implemented).

Kickoff document for the backend repo that will power Slinger cloud sync, collaboration, workspace hosting, and administration.
...
- API server (implemented: Node.js/TypeScript, Fastify)
- PostgreSQL
- Redis
...
Implemented (see the root README): Node.js 22, TypeScript, Fastify, Prisma + PostgreSQL, zod, argon2, jose (JWT).

Still roadmap: Redis, OIDC/OAuth2, policy engine, tracing and metrics.
...
This section defines the first implementation contracts the API should expose.
...
This endpoint list should be treated as the first practical integration target between the API and the Slinger desktop app.
